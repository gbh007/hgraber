...
build: create_build_dir
	CGO_ENABLED=0 GOOS=linux GOARCH=arm64 go build $(LDFLAGS) -o ./_build/hgraber-linux-arm64 ./cmd/simple
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build $(LDFLAGS) -o ./_build/hgraber-linux-amd64 ./cmd/simple
	CGO_ENABLED=0 GOOS=windows GOARCH=amd64 go build $(LDFLAGS) -o ./_build/hgraber-windows-amd64.exe ./cmd/simple
...
run: create_build_dir
	go build $(LDFLAGS) -o ./_build/hgraber-bin  ./cmd/simple
...
debug: create_build_dir
	go build $(LDFLAGS) -trimpath -o ./_build/hgraber-bin  ./cmd/simple
